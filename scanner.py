...
def merge_external_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if not institutional.empty:
        ...
        for col in ["foreign_buy_days", "investment_buy_days", "dealer_buy_days", "foreign_buy", "trust_buy", "dealer_buy", "trust_holding_pct", "estimated_inst_cost"]:
            out[col] = 0.0
...
def calculate_scores(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["score_total"] = out["tech_score"] + out["institution_score"] + out["main_force_score"] + out["broker_score"] + out["breakout_score"] + out["revenue_score"]
...
def build_payload(raw_df: pd.DataFrame, analyzed_df: pd.DataFrame) -> dict:
    ...
    starting_df = analyzed_df[analyzed_df["radar_tag"] == "剛啟動"].sort_values(["score_starting", "score_total"], ascending=False).head(TOP30_COUNT).copy()
    second_wave_df = analyzed_df[analyzed_df["radar_tag"] == "可能第二波"].sort_values(["score_second_wave", "score_total"], ascending=False).head(TOP30_COUNT).copy()
    broker_track_df = analyzed_df.sort_values(["broker_score", "main_force_score", "score_total"], ascending=False).head(BROKER_TRACK_COUNT).copy()

    risk_overheated_df = analyzed_df[(analyzed_df["volume_ratio"] >= 2.5) & (analyzed_df["rsi"] >= 70)].head(20).copy()
    high_turnover_df = analyzed_df.sort_values(["turnover_100m", "volume_ratio"], ascending=False).head(20).copy()
...
def run_scan(save: bool = True) -> dict:
    ...
    analyzed_df = analyzed_df.sort_values(["score_total", "turnover_100m", "volume_ratio"], ascending=False).reset_index(drop=True)
